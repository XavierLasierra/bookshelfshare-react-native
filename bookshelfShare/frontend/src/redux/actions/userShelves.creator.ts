...
export function loadUserShelves(
  userId: string,
  token: string,
  refreshToken: string,
) {
  return async (dispatch: Dispatch) => {
    try {
      const {data} = await axios.get(
        BOOKSS_API.concat(`/shelves?users=${userId}`),
        {
          headers: {
    ...
    try {
      if (deleteFrom) {
        const {data} = await axios.put(
          BOOKSS_API.concat(`/shelves/book/${deleteFrom}`),
          {
            actionType: 'DELETE',
            bookIsbn,
          },
      ...
      if (addTo) {
        const {data} = await axios.put(
          BOOKSS_API.concat(`/shelves/book/${addTo}`),
          {
            actionType: 'ADD',
            bookIsbn,
            customInformation,
