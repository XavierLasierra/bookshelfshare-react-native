...
import {IBookData, IBookResultsProps, IStore} from '../../types/interfaces';

export default function BookResults({
  navigation,
  route: {
    params: {searchInformation, listName, logo},
  },
}: IBookResultsProps) {
  ...
  function renderBook({item}: any) {
    return (
      <BookElementSearch bookData={item} navigation={navigation} logo={logo} />
    );
  }
      ...
      <View style={styles.resultsContainer}>
        {listName ? (
          <BookListFilter
            listName={listName}
            books={books}
            setFilteredBooks={setFilterdBooks}
          />
        ) : (
          <View style={styles.searchParameters}>
            <Text style={styles.parameter}>
              {searchInformation?.isbn &&
                `ISBN: ${searchInformation?.isbn.toUpperCase()}`}
            </Text>
            <Text style={styles.parameter}>
              {searchInformation?.intitle &&
                `Title: ${searchInformation?.intitle.toUpperCase()}`}
            </Text>
            <Text style={styles.parameter}>
              {searchInformation?.inauthor &&
                `Author: ${searchInformation?.inauthor.toUpperCase()}`}
            </Text>
            <Text style={styles.parameter}>
              {searchInformation?.inpublisher &&
                `Publisher: ${searchInformation?.inpublisher.toUpperCase()}`}
            </Text>
          </View>
        )}
        {results ? (
