...
export default function UsersRatings({ratings, userId}: any) {
  ...
  function countNumberOfRatings(rating: number) {
    return ratings.reduce(
      (acc: number, {rating: currentRating}: any) =>
        currentRating === rating ? acc + 1 : acc,
      0,
    );
  }

  function handleFilter(rating: number) {
    const newFilteredRatings = ratings.filter(
      ({rating: currentRating}: any) => currentRating === rating,
    );
  ...
  return (
    <View style={styles.usersReviewsContainer}>
      <Text style={[globalStyles.titleText, styles.ratingTitle]}>
        Users reviews:
      </Text>
        ...
        {ratingTypes.map(ratingNumber => (
          <TouchableOpacity
            ...
            <Text style={styles.reviewNumberText}>
              {countNumberOfRatings(ratingNumber)} reviews
            </Text>
          </TouchableOpacity>
        ))}
        <TouchableOpacity
      ...
      {filteredRatings.length > 0 ? (
        filteredRatings.map((rating: any) =>
          rating?.user?._id !== userId ? (
            <RatingElement
              key={`ratingNumber-${rating?.user?._id}`}
              rating={rating}
            />
          ) : (
            <RatingElement
        ...
        <Text testID="noRatings" style={styles.allReviewsText}>
          0 reviews
        </Text>
      )}
    </View>
