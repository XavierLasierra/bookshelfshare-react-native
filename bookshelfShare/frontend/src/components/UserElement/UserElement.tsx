...
export default function UserElement({
  ...
  const isFollowed = following.some(
    (followedUser: any) => followedUser._id === user._id,
  );
  ...
  function handleAddFollowing(followingUserId: string) {
    dispatch(
      addUserFollowing(followingUserId, loggedUserId, token, refreshToken),
    );
    setIsDisabled(true);
  }
  ...
  function handleDeleteFollowing(followingUserId: string) {
    dispatch(
      deleteUserFollowing(followingUserId, loggedUserId, token, refreshToken),
    );
    setIsDisabled(true);
  }
      ...
      {user._id !== loggedUserId && (
        <TouchableOpacity
          style={[
            ...
            isFollowed && styles.userElementFollowedContainer,
          ]}
